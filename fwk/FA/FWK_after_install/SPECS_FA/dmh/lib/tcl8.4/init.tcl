...
proc auto_load_index {} {
	...
	set f ""
	if {$issafe} {
            if {[catch {source [file join $dir tclIndex]}]} {
                catch { source [file join $dir tclindex.txt]}
                }
	} else {
            if {[catch {set f [open [file join $dir tclIndex]]}]} {
                if {[catch {set f [open [file join $dir tclindex.txt]]}]} continue
                }
	    set error [catch {
		set id [gets $f]
		if {[string equal $id \
...
proc auto_execok name {
    ...
    set shellBuiltins [list cls copy date del erase dir echo mkdir \
	    md move rename ren rmdir rd time type ver vol]
    if {[string equal $tcl_platform(os) "Windows NT"]} {
