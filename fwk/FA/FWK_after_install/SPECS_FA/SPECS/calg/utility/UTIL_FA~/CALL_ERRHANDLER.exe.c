#include <SPECS.h>
#include <SPECS_SYS.h>
#include <SPECS_EXE.h>

void
_CALL_ERRHANDLER(void *_alg, void *__alg)
/* _alg points to ExeSpecDefRow */
/* __alg points to AlgorithmSpecDefIndexEntry */
{
	/* P-code pointer definition */
	void *_in, *__in;
	void *_out, *__out;

	/* Local input/output variable definition */
	STRING	ErrorNo;
	STRING	Message;
	STRING	MoreInfo;
	INTEGER	Status;
	STRING	Errmsg;

	/* P-code pointer initialization */
	_in = TPLEXECIN(_alg);
	__in = TPLLASTIN(__alg);
	_out = TPLEXECOUT(_alg);
	__out = TPLLASTOUT(__alg);

	/* Local variable initialization */
	TPLINITMEM(&ErrorNo, _TYPES, 0, 0);
	TPLINITMEM(&Message, _TYPES, 0, 0);
	TPLINITMEM(&MoreInfo, _TYPES, 0, 0);
	TPLINITMEM(&Status, _TYPEI, 0, 0);
	TPLINITMEM(&Errmsg, _TYPES, 0, 0);

	/* Algorithm input variable setting */
	TPLGETEXECINVAR(_in, 0, &ErrorNo);
	TPLGETEXECINVAR(_in, 1, &Message);
	TPLGETEXECINVAR(_in, 2, &MoreInfo);
	TPLPUTLASTINVAR(__in, 0, &ErrorNo);
	TPLPUTLASTINVAR(__in, 1, &Message);
	TPLPUTLASTINVAR(__in, 2, &MoreInfo);

	/* Algorithm output variable setting */
	TPLGETEXECOUTVAR(_out, 0, &Status);
	TPLGETEXECOUTVAR(_out, 1, &Errmsg);
	TPLPUTLASTOUTVAR(__out, 0, &Status);
	TPLPUTLASTOUTVAR(__out, 1, &Errmsg);

	/* Algorithm invocation */
	CALL_ERRHANDLER(ErrorNo,Message,MoreInfo,&Status,&Errmsg);

	/* Algorithm output variable setting */
	TPLPUTEXECOUTVAR(_out, 0, &Status);
	TPLPUTEXECOUTVAR(_out, 1, &Errmsg);
	TPLPUTLASTOUTVAR(__out, 0, &Status);
	TPLPUTLASTOUTVAR(__out, 1, &Errmsg);
}

void
__CALL_ERRHANDLER(void *__alg)
/* __alg points to AlgorithmSpecDefIndexEntry */
{
	/* P-code pointer definition */
	void *__in;
	void *__out;

	/* Local input/output variable definition */
	STRING	ErrorNo;
	STRING	Message;
	STRING	MoreInfo;
	INTEGER	Status;
	STRING	Errmsg;

	/* P-code pointer initialization */
	__in = TPLLASTIN(__alg);
	__out = TPLLASTOUT(__alg);

	/* Local variable initialization */
	TPLINITMEM(&ErrorNo, _TYPES, 0, 0);
	TPLINITMEM(&Message, _TYPES, 0, 0);
	TPLINITMEM(&MoreInfo, _TYPES, 0, 0);
	TPLINITMEM(&Status, _TYPEI, 0, 0);
	TPLINITMEM(&Errmsg, _TYPES, 0, 0);

	/* Algorithm input variable setting */
	TPLGETLASTINVAR(__in, 0, &ErrorNo);
	TPLGETLASTINVAR(__in, 1, &Message);
	TPLGETLASTINVAR(__in, 2, &MoreInfo);

	/* Algorithm output variable setting */
	TPLGETLASTOUTVAR(__out, 0, &Status);
	TPLGETLASTOUTVAR(__out, 1, &Errmsg);

	/* Algorithm invocation */
	CALL_ERRHANDLER(ErrorNo,Message,MoreInfo,&Status,&Errmsg);

	/* Algorithm output variable setting */
	TPLPUTLASTOUTVAR(__out, 0, &Status);
	TPLPUTLASTOUTVAR(__out, 1, &Errmsg);
}
