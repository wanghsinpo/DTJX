...
void
_JDG_WAF_BY_PARA(void *_alg, void *__alg)
...
/* __alg points to AlgorithmSpecDefIndexEntry */
{
	...
	TPLGETEXECINVAR(_in, 0, &Tgt_yld);
	TPLGETEXECINVAR(_in, 1, &Last);
	TPLGETEXECINVAR(_in, 2, &Critical);
	TPLGETEXECINVAR(_in, 3, &Ignore);
	TPLPUTLASTINVAR(__in, 0, &Tgt_yld);
	TPLPUTLASTINVAR(__in, 1, &Last);
	TPLPUTLASTINVAR(__in, 2, &Critical);
	TPLPUTLASTINVAR(__in, 3, &Ignore);
	...
	JDG_WAF_BY_PARA(Tgt_yld,Last,Critical,Ignore,&Jdg,&Yld,&Pass_cnt,&Fail_cnt,&Total_cnt);
...
/* __alg points to AlgorithmSpecDefIndexEntry */
{
	...
	/* Algorithm input variable setting */
	TPLGETLASTINVAR(__in, 0, &Tgt_yld);
	TPLGETLASTINVAR(__in, 1, &Last);
	TPLGETLASTINVAR(__in, 2, &Critical);
	TPLGETLASTINVAR(__in, 3, &Ignore);
