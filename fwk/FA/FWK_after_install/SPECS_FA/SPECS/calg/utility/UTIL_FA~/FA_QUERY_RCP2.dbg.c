...
void
_FA_QUERY_RCP2(void *_alg)
{
}

void
__FA_QUERY_RCP2(void *_alg)
/* _alg points to AlgorithmDefinition */
{
	...
	TPLGETDEBUGOUTVAR(_out, 10, &Mlb);
	TPLGETDEBUGOUTVAR(_out, 11, &Lt_id);
	TPLGETDEBUGOUTVAR(_out, 12, &Pt_id);
	TPLGETDEBUGOUTVAR(_out, 13, &Ps_id);
	TPLGETDEBUGOUTVAR(_out, 14, &Item);
	TPLGETDEBUGOUTVAR(_out, 15, &Status);
	TPLGETDEBUGOUTVAR(_out, 16, &Errmsg);
	...
	TPLPUTDEBUGOUTVAR(_out, 10, &Mlb);
	TPLPUTDEBUGOUTVAR(_out, 11, &Lt_id);
	TPLPUTDEBUGOUTVAR(_out, 12, &Pt_id);
	TPLPUTDEBUGOUTVAR(_out, 13, &Ps_id);
	TPLPUTDEBUGOUTVAR(_out, 14, &Item);
	TPLPUTDEBUGOUTVAR(_out, 15, &Status);
	TPLPUTDEBUGOUTVAR(_out, 16, &Errmsg);
}
