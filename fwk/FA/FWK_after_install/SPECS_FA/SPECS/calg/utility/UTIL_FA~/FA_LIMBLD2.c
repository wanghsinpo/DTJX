...
--- --------------- ---- -------- --------------------------------
  ...
  8 Lms             S    256      Limit spec name to write        
...
void
FA_LIMBLD2(INTEGER Rev_flag,STRING Brk_chr,STRING Tpl_dir,STRING Lim_dir,STRING Lms_dir,STRING Tpl,STRING Lim,STRING *Lms,INTEGER *Status,STRING *Error,INTEGER *Ndetail,STRING *Detail)
{
...
#define LMS_SIZE 256
	...
	pl = (char *)strtok_r(pl, Brk_chr, &last);
	    ...
	    FA_CHK_FILES(Rev_flag, Lms_dir, Lms, "lms", Brk_chr, &Stat);
            strcat(cmd, " -lms ");
            strcat(cmd, Lms_dir);  /* Add 2005/090/02 */
            strcat(cmd, Lms_dir[strlen(Lms_dir)-1]=='/' ? "" : "/");  /* Add 2005/090/02 */
            strcat(cmd, Lms[i]);
        }
        ...
        sprintf(t_cmd, "%s %s %s > %s 2> %s", cmd, ls, t, l, err);
/* Bugfix 2005/09/02
        sprintf(t_cmd, "%s %s %s > %s 2> %s", cmd, t, ls, l, err);
*/
