...
void
FA_CHK_FILES2(INTEGER Rev_flag,STRING Dir,STRING Filename_rev,STRING Suffix,STRING Brk_chr,INTEGER *Status,STRING *File_name,STRING *Full,STRING *Rev)
{
...
#define	CMD	"/opt/SPECS/sys/bin/revopen"
	...
	strcpy(*File_name, ""); /* Default value is null string '01/10/10 */
	...
	*Status=0;
	...
	/* Make file name again '01/10/10 */
	sprintf((char *)File_name, "%s.%s", file, suf); 
	...
	pclose(fp);
/* 2006/02/10 */
        strcpy((char *)Full,full);
	rev	= (char *)strtok_r(buf, "\t", &last);
	rev	= (char *)strtok_r(NULL, "\n", &last);
        strcpy((char *)Rev,rev);
