...
void
_PROBER_TEMPSET(void *_alg, void *__alg)
...
/* __alg points to AlgorithmSpecDefIndexEntry */
{
	...
	TPLINITMEM(&Settemp, _TYPER, 0, 0);
	TPLINITMEM(&Status, _TYPES, 0, 0);
	TPLINITMEM(&Error, _TYPES, 0, 0);

	/* Algorithm input variable setting */
	TPLGETEXECINVAR(_in, 0, &Settemp);
	TPLPUTLASTINVAR(__in, 0, &Settemp);
	...
	PROBER_TEMPSET(Settemp,&Status,&Error);
...
/* __alg points to AlgorithmSpecDefIndexEntry */
{
	...
	/* Algorithm input variable setting */
	TPLGETLASTINVAR(__in, 0, &Settemp);
